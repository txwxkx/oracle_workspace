...
WHERE column_name = value;
*/
DELETE FROM emp01 --DELETE와 FROM 사이에는 아무것도 들어갈 수 없다.
WHERE first_name = 'candy';
...
/* 자동 카밋 설정 확인(SQL Developer)
도구 > 환경설정 > 데이터베이스 > 객체뷰어 > 행 변경 시 사후 편집, 자동 커밋 설정 체크 해제
*/
...
ROLLBACK; --롤백 하는 순간 waiting 중이던 RSCL에서 1 row created 됨.
SELECT * FROM emp02;
