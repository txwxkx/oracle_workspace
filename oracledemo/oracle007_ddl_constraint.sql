...
--------------------------------------------------------------------------
--3000 : 크기 초과(3자리까지만 가능 - 0 ~ 999)
--ORA-01438: value larger than specified precision allowed for this column
INSERT INTO student(name, age, avg, hire)
VALUES('홍길동', 3000, 97.85, sysdate);

--2997.85 : 크기 초과(5자리까지만 가능) - 소수점은 포함되지 않는다.
--ORA-01438: value larger than specified precision allowed for this column
INSERT INTO student(name, age, avg, hire)
VALUES('홍길동', 30, 2997.8589, sysdate);
...
SELECT * FROM student;
------------------------------------------------------------------------
--소수점 2자리로 무조건 계산을 한다. 즉 점수는 3자리까지만 허용한다.
--ORA-01438: value larger than specified precision allowed for this column
INSERT INTO student(name, age, avg, hire)
...
ROLLBACK; --TRUNCATE TABLE은 자동으로 커밋되기 때문에 롤백해도 복구 안됨. 사용시 주의!
...
--emp08 테이블의 deptno 컬럼에서 30을 참조하지 않기 때문에 이상없이 삭제가 가능하다.
DELETE FROM dept01
WHERE deptno = 30;
...
/*
부모키가 삭제가 되면 참조 되는 키도 삭제가 되도록 cascade를 설정한다.
*/
...
/*
ON UPDATE CASCADE는 오라클에서 제공 안됨
해결방법 : trigger
*/
